...
# -*- coding: utf-8 -*-
"""
SVS13A streamer fitting script（整理版）
...
cube_fname = "DCN_channel_map_Tb.fits"

CACHE_DIR = "SVS13A_results/cache"
os.makedirs(CACHE_DIR, exist_ok=True)
PLOT_DIR = "SVS13A_results/plots"
os.makedirs(PLOT_DIR, exist_ok=True)

CACHE_PATH_GRID = os.path.join(CACHE_DIR, "SVS13A_grid_results.npz")
CACHE_PATH_MCMC_GRID = os.path.join(CACHE_DIR, "SVS13A_mcmc_grid_results.npz")
# CACHE_PATH_MCMC_DISTANCE = os.path.join(CACHE_DIR, "SVS13A_mcmc_distance_results.npz")
CACHE_PATH_MCMC_SHELL = os.path.join(CACHE_DIR, "SVS13A_mcmc_shell_results.npz")
CACHE_PATH_FINAL = os.path.join(CACHE_DIR, "SVS13A_fit_results_final.npz")
...
def plot_z_v_diagram_from_cube(theta_deg, phi_deg, inc_deg, T_Myr, omega,
                               ...
                               outname,
                               label="SVS13A DCN z-v with data"):
    """
...
if RUN_FROM_CACHE_ONLY:
    ...
    try:
        ...
        try:
            str_mom0 = fits.getdata("SVS13A_DCN_streamer_mom0.fits")
            str_mom1 = fits.getdata("SVS13A_DCN_streamer_mom1.fits")
            cube = fits.getdata("SVS13A_CD_l021l060_uvsub_DCN_multi_small_fitcube.fits")
            header = fits.getheader("SVS13A_DCN_streamer_mom1.fits")
        except Exception:
        ...
        new_cube_data = None
        try:
            new_cube_data = fits.getdata("SVS13A_DCN_streamer_cube.fits")
        ...
        plot_z_v_diagram_from_cube(
            ...
            outname="SVS13A_z_v_data_overlay.png",
        )
        ...
        plot_streamer_on_mom1(
            ...
            header, pa_rad, dx_au, im_center,
            str_mom1,
            label='SVS13A '+r'$\rm DCN$',
            outname="SVS13A_mom1_cacheonly.png",
            cen_x_pix=cen_x_pix,
            cen_z_pix=cen_z_pix,
        ...
        plot_streamer_on_mom0(
            ...
            str_mom0,
            label='SVS13A '+r'$\rm DCN$',
            outname="SVS13A_mom0_cacheonly.png",
            cen_x_pix=cen_x_pix,
            cen_z_pix=cen_z_pix,
            cen_v_LS_km=cen_v_LS,
        )
...
def prepare_data():
    ...
    fits.PrimaryHDU(data=moment0, header=header).writeto(
        "SVS13A_DCN_mom0.fits", overwrite=True
    )
    h1 = fits.PrimaryHDU(data=moment1.data, header=header)
    h1.header["BUNIT"] = "km/s"
    h1.writeto("SVS13A_DCN_mom1.fits", overwrite=True)
    ...
    try:
        fits.PrimaryHDU(data=new_cube_data, header=header).writeto(
            "SVS13A_DCN_streamer_cube.fits", overwrite=True
        )
    ...
    fits.PrimaryHDU(data=str_mom0, header=header).writeto(
        "SVS13A_DCN_streamer_mom0.fits", overwrite=True
    )
    fits.PrimaryHDU(data=str_mom1, header=header).writeto(
        "SVS13A_DCN_streamer_mom1.fits", overwrite=True
    )
...
def run_final_best_fit_and_overlay():
    try:
        ...
        plot_streamer_on_mom1(
            ...
            header, pa_rad, dx_au, im_center,
            str_mom1,
            label="SVS13A DCN moment1 (best-fit)",
            outname="SVS13A_model_vs_mom1_overlay_best.png",
            cen_x_pix=cen_x_pix,
            cen_z_pix=cen_z_pix,
            cen_v_LS_km=cen_v_LS,
        )
        ...
        plot_streamer_on_mom0(
            ...
            header, pa_rad, dx_au, im_center,
            str_mom0,
            label="SVS13A DCN moment0 (best-fit)",
            outname="SVS13A_model_vs_mom0_overlay_best.png",
            cen_x_pix=cen_x_pix,
            cen_z_pix=cen_z_pix,
            cen_v_LS_km=cen_v_LS,
        )
