...
Each MCMC run begins by initializing an ensemble of walkers within the prior intervals, distributed in a narrow region around the best–performing grid–search solution. For the likelihood function based on the 11 position and velocity centroids, we typically use $N_{\rm walkers}=20$ and $N_{\rm steps}=2\times10^4$, initializing the walkers around the best grid solution but clipping them to the prior ranges. This “fast–likelihood’’ stage provides an efficient refinement of the parameter space and helps to identify the dominant modes of the posterior distribution.

For the full datacube likelihood, we evaluate the agreement between the model and the observed Position–Position–Velocity structure using a shell–based distance metric. Instead of computing distances for every voxel, the method constructs a shell–distance cube around the model streamer and compares it with the data cube through intensity–weighted shell distances. For this shell-based likelihood, we adopt $N_{\rm walkers}=20$, and $N_{\rm steps}=8\times10^{3}$. This stage allows the MCMC to probe the global three–dimensional structure while remaining computationally tractable and provides a more restrictive, global constraint on the model parameters.
