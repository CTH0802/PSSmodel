...
FRAMES = 8
FPS = 2
R_vis = 3e3  # 視覺化用縮放範圍 (AU) 3e3(-50), 1.3e4(-2)
...
# T_Myr     = float(cache["best_T"])
# omega     = float(cache["best_Omega"])
M_star    = float(cache.get("M_star", 2.58))  # 若沒存 M_star 則預設 3.2
r_in_au   = float(cache.get("r_in_au", 2e2))
r_out_au  = float(cache.get("r_out_au", 3.8e3))
...
def update(frame):
    ...
    pa_rad = np.deg2rad(90)
    # x_m = x_m * np.cos(pa_rad) - z_m * np.sin(pa_rad)
    # z_m = x_m * np.sin(pa_rad) + z_m * np.cos(pa_rad)
